...
SCHOOL_KEEP_WORDS = ["holiday", "break", "early release", "no school", "first day", "last day", "workday", "teacher", "recess", "dismiss", "closed", "student"]
...
def school():
    try:
        ...
        return Response(f"Failed to fetch school calendar: {e}", status=502)
    filtered = filter_school_ical(r.text, SCHOOL_KEEP_WORDS)
    return Response(filtered, mimetype="text/calendar")

@app.route("/")
def index():
    return "Ride filter is running. Feeds: /feed (rides) | /school (school calendar)"
